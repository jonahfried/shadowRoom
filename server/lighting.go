...
func illuminate(room Place, cir Agent, canvas *pixelgl.Window) {
	shadowImg := imdraw.New(nil)
	shadowImg.Color = colornames.Black
	...
	for _, block := range room.Blocks {
		...
		shadePointsByViewMode(obstructedPoints, shadowImg, cir)
		shadeObstructedPointsToCorners(obstructedPoints, shadedRoomCorners, shadowImg, cir)
